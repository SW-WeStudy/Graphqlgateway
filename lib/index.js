...
const url$4 = '54.92.227.88';
const port$4 = '3000';
const entryPoint$3 = 'studyrooms';
const entryPointuc = 'usercreates';

const URL$5 = `http://${url$4}:${port$4}/${entryPoint$3}`;
const resolvers$5 = {
	Query: {
		get_study_rooms: (_,) => {	
			return generalRequest(`${URL$5}`, 'GET')
		},
		get_study_room: (_, {sr_id }) => {	
			return generalRequest(`${URL$5}/${sr_id}`, 'GET')
		},
		get_resources: (_, {sr_id}) => {
			return generalRequest(`${URL$5}/${sr_id}/resources`, 'GET')
		},
		get_resource: (_, {_id, sr_id }) => {
			return generalRequest(`${URL$5}/${sr_id}/resources/${_id}`, 'GET')
		},
		get_students: (_, {sr_id}) => {
			return generalRequest(`${URL$5}/${sr_id}/students`, 'GET')
		},
	...
	Mutation: {
		...
		create_study_room: (_, { studyroom }) =>{
			return generalRequest(`${URL$5}`, 'POST', studyroom)
		},
		delete_study_rooms: (_,) => {
			return generalRequest(`${URL$5}`, 'DELETE') // Probar de últimas.
		},
		update_study_room: (_, { sr_id, studyroom }) => {
			return generalRequest(`${URL$5}/${sr_id}`, 'PUT', studyroom)
		},
		delete_study_room: (_, {sr_id}) => {
			return generalRequest(`${URL$5}/${sr_id}`, 'DELETE')
		},



		create_resource: (_, {sr_id, resource }) =>{
			return generalRequest(`${URL$5}/${sr_id}/resources`, 'POST', resource)
		},
		delete_resources: (_, {sr_id}) => {
			return generalRequest(`${URL$5}/${sr_id}/resources`, 'DELETE') 
		},
		update_resource: (_, { sr_id, _id, resource }) => {
			return generalRequest(`${URL$5}/${sr_id}/resources/${_id}`, 'PUT', resource)
		},
		delete_resource: (_, {sr_id, _id}) => {
			return generalRequest(`${URL$5}/${sr_id}/resources/${_id}`, 'DELETE')
		},


		inscribe_student: (_, {sr_id, student}) => {
			return generalRequest(`${URL$5}/${sr_id}/students`, 'POST', student)
		},
		delete_student: (_, {sr_id, rmstudent}) => {
			return generalRequest(`${URL$5}/${sr_id}/students`, 'DELETE', rmstudent) 
		},
...
const URL$6 = `http://${url$4}:${port$4}/${entryPointuc}`;

const resolvers$6 = {
    Query: {
        get_srs_created_by_user: (_, {user}) => {
            return generalRequest(`${URL$6}`, 'GET',{user})
        }
    },
    Mutation: {
        delete_srs_created_by_user: (_, {user}) => {
            return generalRequest(`${URL$6}`,'DELETE',{user})
        },
    }
};

const url$5 = '34.224.170.220';
const port$5 = '5000';

const URL$7 = `http://${url$5}:${port$5}`;


const resolvers$7 = {
	Query: {
		GetAllUserCourse: (vasd) => {	
			return generalRequest(`${URL$7}/class/admin/usercourses`, 'GET')
		}
	},
};
