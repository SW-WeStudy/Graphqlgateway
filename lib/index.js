...
const url$1 = '34.224.170.220';
const port$1 = '8080';
const entryPoint$1 = 'course';

const URL$1 = `http://${url$1}:${port$1}/${entryPoint$1}/notes`;


const resolvers$1 = {
	Query: {
		allComentsOfNote: (_, { id }) => {	
			return generalRequest(`${URL$1}/getcomments/${id}`, 'GET')
		}
	},
	Mutation: {
		createComment: (_, { comment }) =>{
			return generalRequest(`${URL$1}/comment/create`, 'POST', comment)
		},
        updateComment: (_, { id, comment }) =>
			generalRequest(`${URL$1}/comment/edit/${id}`, 'PUT', comment),
        deleteCategory: (_, { id }) =>
			generalRequest(`${URL$1}/comment/delete/${id}`, 'DELETE')
	}
};

const URL$2 = `http://${url$1}:${port$1}/${entryPoint$1}/resources`;


const resolvers$2 = {
	Query: {
		allResourcesOfClass: (_, { id }) => {	
			return generalRequest(`${URL$2}/get/${id}`, 'GET')
		}
	},
	Mutation: {
		createResource: (_, { resource }) =>{
			return generalRequest(`${URL$2}/create`, 'POST', resource)
		},
        updateResource: (_, { id, resource }) =>
			generalRequest(`${URL$2}/edit/${id}`, 'PUT', resource),
        deleteResource: (_, { id }) =>
			generalRequest(`${URL$2}/delete/${id}`, 'DELETE')
	}
};
